...
void loop() {
  ...
  int pwmValue = 1500 + (int)(500 * error);  // Adjust the scaling factor as needed
...
//   Move the motor backward
 digitalWrite(in1, LOW);
 digitalWrite(in2, HIGH);
 analogWrite(enA, 1500);  // Adjust PWM value for desired speed
 delay(4000);  // Run for 4 seconds
 ...
 delay(1000);  // Pause for 1 second
  ...
  // Check for flow rate
  if (currentFlowRate != desiredFlowRate) {  // Adjust the threshold as needed
    Serial.println("Current flow rate < Desired flow rate!");
    digitalWrite(buzzerPin, HIGH);  // Turn the buzzer on
  ...
  if (batteryVoltage < 4.9) {  // Adjust the threshold as needed
    Serial.print("Low battery!, battery voltage is: ");
